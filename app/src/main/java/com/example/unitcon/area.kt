...
class area : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        editInput.addTextChangedListener(){
            ...
            if(inputVal.isNotEmpty())
            {
                ...
                if(selectedUnit=="Meter Square"){
                    resultText =df.format(doubleInput*10000)
                    resultTypeText.text = "Centimeter Square"
                }
                else{
                    resultText = df.format((doubleInput/10000.0))
                    resultTypeText.text = "Meter Square"
                }
    ...
    private fun showAlertDialog() {
        var alertDialog : AlertDialog.Builder = AlertDialog.Builder(this@area)
        alertDialog.setTitle("Select Input Unit")
        val items = arrayOf("Meter Square","Centimeter Square")
        val checkedItems=-1
